...
```{r}
pnorm(160,180,25,lower.tail=TRUE) + pnorm(192,180,25,lower.tail=FALSE)
```

The probabilty that a random draw from N(180,25^2) will be less than 192 and greater than 160 is p(160 < Z < 192) = `r pnorm(160,180,25,lower.tail=TRUE) + pnorm(192,180,25,lower.tail=FALSE)` (Remember to set your tails correctly so you find 1 - area_left_of_160 - area_right_of_192)
...
The probabilty that X is greater than or equal to -1 is $P(x>=-1)$ = 0.15+0.35+0.25+.10 = `r 0.15+0.35+0.25+0.10`. 
...
10. Consider a population of donuts with with an average survival time until becoming stale of $\mu = 3$ days and a standard deviation of failure times $\sigma = 0.25$ days (exponential distribution). If you take a sample of 24 donuts, what are the chances the mean failure time of your sample will be:

(a) Greater than 3 days? 

To compute this, use the helpfile (`?pexp`, or any other in the `rexp`/`dexp`/`qexp` family), which will show you that the rate parameter is given by $mean = 1/rate$. Thus, since $\mu = 3$, we should set the rate equal to $1/3$. To prove this, I'll quickly simulate 10,000 draws form this distribution; the mean of the draws should be about equal to 3:

```{r}
mean(rexp(10000,1/3))
```

Anyways, we can use `pexp` to calculate the probablity that the mean failure time given a 24-donut sample will be greater than 3:

```{r}
pexp(3,rate = 1/3,lower.tail=FALSE)
```

Note that this distribution is NOT a function of sample size, so this does not actually factor in.
